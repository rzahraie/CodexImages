...
namespace NinjaTrader.Indicator
{
    ...
    public class TChannelV6 : Indicator
    {
		...
		private string GetVolumeSeq(int Bar)
		{
			...
			if ((IsHigherHighHigherLow(Bar-1,Bar)) && (GetClose(Bar) < GetClose(Bar-1))) s = s + "*";
			else if ((IsLowerHighLowerLow(Bar-1,Bar)) && (GetClose(Bar) > GetClose(Bar-1))) s = s + "*";
			else if ((IsFTP(Bar-1,Bar)) && (GetClose(Bar) < GetClose(Bar-1))) s = s + "*";
			else if ((IsFBP(Bar-1,Bar)) && (GetClose(Bar) > GetClose(Bar-1))) s = s + "*";
...
//@@
public enum ChannelTapeJoinCriteria
{
	JoinByTicks,
	CloseEqualHighOrLow,
	OpenEqualsHighOrLow,
